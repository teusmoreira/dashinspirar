...
with st.sidebar:
    ...
    # Se não tiver upload, usa mensagem informativa
    if not uploaded_file:
         st.info("Usando dados padrão ou aguardando upload.")
...
if uploaded_file is not None:
    ...
    try:
        ...
        if raw_df is not None:
            st.sidebar.success("✅ Dados locais carregados")
    except:
        st.sidebar.warning("⚠️ Arquivo local não encontrado")

# --- LÓGICA DE FILTRO E CONSTRUÇÃO DO DASHBOARD ---
if raw_df is not None:
    
    # 1. Preparar datas mínima e máxima disponíveis no dataset
    min_date = raw_df["createdAt"].min().date()
    max_date = raw_df["createdAt"].max().date()

    # 2. Criar o Widget de Filtro na Sidebar
    with st.sidebar:
        st.divider()
        ...
        date_range = st.date_input(
            ...
            value=(min_date, max_date), # Valor padrão: tudo
            min_value=min_date,
            max_value=max_date
        )

    # 3. Aplicar o Filtro no DataFrame
    # Se o usuário selecionar apenas uma data, o Streamlit retorna apenas um valor, não uma tupla
    if len(date_range) == 2:
        start_date, end_date = date_range
        mask = (raw_df['createdAt'].dt.date >= start_date) & (raw_df['createdAt'].dt.date <= end_date)
        df = raw_df.loc[mask] # Cria o 'df' final filtrado
    else:
        df = raw_df.copy() # Se estiver selecionando, mantém tudo por enquanto

    # -----------------------------------------------
    # DAQUI PRA BAIXO, TODO O CÓDIGO USA 'df' (JÁ FILTRADO)
    # -----------------------------------------------
    ...
    # Evitar divisão por zero se o filtro for muito restritivo
    if total_pacientes > 0:
        pct_ativos = (len(ativos) / total_pacientes) * 100
    else:
        pct_ativos = 0

    col1.metric("👥 Total Filtrado", total_pacientes)
    ...
    if total_pacientes == 0:
        ...
        with tab2:
            ...
            if not ativos_sexo.empty:
                ...
                st.info("Sem dados de gênero para o filtro selecionado.")
        ...
        with tab3:
                ...
                for _, row in df.iterrows():
                    if isinstance(row[col_name], list):
                        for log in row[col_name]:
                            # IMPORTANTE: Filtrar os logs individuais também pela data selecionada
                            log_date = pd.to_datetime(log.get('createdAt'), errors='coerce')
                            if pd.notnull(log_date):
                                if len(date_range) == 2:
                                    start, end = date_range
                                    if start <= log_date.date() <= end:
                                        lista_log.append({'date': log_date, 'Funcionalidade': feat_name})
                                else:
                                    lista_log.append({'date': log_date, 'Funcionalidade': feat_name})
            ...
            if not df_logs.empty:
                ...
                # Reutilizar df_logs que já está filtrado
                ...
                st.info("Sem dados temporais suficientes neste período.")
