...
with st.sidebar:
    ...
    # Removi os containers de texto (st.info/st.warning) daqui

# Caminho fixo local (Mantido o original conforme pedido)
...
# Lógica de Carregamento Inicial (raw_df)
raw_df = None
if uploaded_file is not None:
    ...
    try:
        ...
        pass # Falha silenciosa se não achar arquivo, sem mostrar erro na tela
...
# --- CONSTRUÇÃO DO DASHBOARD (Usa 'df' já filtrado) ---
if df is not None:
    # KPI Section
    ...
    # Previne divisão por zero se filtro não retornar nada
    ...
    if total_pacientes == 0:
        ...
        with tab3:
                ...
                for _, row in df.iterrows():
                    if isinstance(row[col_name], list):
                        for log in row[col_name]:
                            ...
                            if pd.notnull(d_log):
                                # Lógica para filtrar o log individual pela data selecionada também
                                # Isso garante que o gráfico temporal obedeça o filtro da sidebar
                                if len(date_range) == 2:
                                    if start_date <= d_log.date() <= end_date:
                                        lista_log.append({'date': d_log, 'Funcionalidade': feat_name})
                                else:
                                    lista_log.append({'date': d_log, 'Funcionalidade': feat_name})
            ...
            if not df_logs.empty:
                ...
                # Usa os logs filtrados para o mapa de calor também
                ...
                st.info("Sem dados temporais suficientes para o período selecionado.")
    ...
    # Caso nenhum dado seja carregado (sem arquivo local e sem upload)
